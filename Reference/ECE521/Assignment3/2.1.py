...
def load_data():
    #return [ [data] for data in np.load('data100D.npy')] 
...
if __name__ == '__main__':
    ...
    for i in range(epochs):
        _,loss, mean = sess.run([train,total_loss, mu_T], feed_dict = {x:data})
        plot1_x.append(i)
        plot1_y.append(loss)        
    #plot1
    plt.figure(1) 
    ...
    maxs = sess.run(argmaxs , feed_dict = {x:data})
    for i in range(len(data)):
        plot2_x[maxs[i]].append(data[i][0][0])
        plot2_y[maxs[i]].append(data[i][0][1])
    ...
    print("data points belonging to cluster"+ str(temp+1) + " = " + str(count) + "\n") 
    print("percetage = " + str(count/B) + "\n")
    mean, prob_z, variance = sess.run([mu, pi, var], feed_dict = {x:data})
