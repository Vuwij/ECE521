...
def compute_weighted_sum(previous_layer, n_units):
    ...
    #bias (broadcasted)
    bias = tf.Variable(tf.zeros(shape=[1,n_units[1]]), [1,n_units[1]], name="bias")
...
def build_neural_network():
    ...
    n_hidden_1 = 3000 # 1st layer number of features
...
if __name__ == '__main__':
    ...
    before_train_acc, train_cost, t_predicted, train_err = sess.run([accuracy, loss_CE, result, error], feed_dict= {X: input_x, target: input_y})  
    ...
    # min cost keep track
    min_valid_cost = train_cost*100;
    min_cost_index = 0; 
    
    # min error keep track
    min_valid_error = 100;
    min_error_index = 0;     
    ...
    for i in range(epoch):
        ...
        if(i%display_step==0):    
            ...
            # set minimum validation cost
            if min_valid_cost > valid_cost:
                min_valid_cost = valid_cost
                min_cost_index = i
                
            # set valid acc
            if min_valid_error > valid_err:
                min_valid_error = valid_err
                min_error_index = i  
    ...
    #finished training; output min cost
    print("Finished Training. The min valid cost is %s at epoch %s"%(min_valid_cost, min_cost_index))
    print("Finished Training. The min valid err is %s at epoch %s"%(min_valid_error, min_error_index))
