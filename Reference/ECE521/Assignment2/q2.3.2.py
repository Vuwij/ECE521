...
def compute_weighted_sum(previous_layer, n_units):
    ...
    #bias (broadcasted)
    bias = tf.Variable(tf.zeros([1,n_units[1]], dtype=tf.float32), [1,n_units[1]], name="bias")
...
def build_neural_network():
    ...
    weight_matrices = [ [[]] for i in range(3) ]
    ...
    hidden_layer_1 = tf.nn.relu(Z[0]) 
    
    #hidden_layer_2 
    Z[1], weight_matrices[1] = compute_weighted_sum(hidden_layer_1, (n_hidden_1, n_hidden_2) )
    hidden_layer_2 = tf.nn.relu(Z[1])    
    
    #output_layer
    Z[2], weight_matrices[2] = compute_weighted_sum(hidden_layer_2, (n_hidden_2, n_classes) )
    Y_predicted =  tf.nn.softmax(Z[2]) 
    
    # cross entropy loss
    loss_CE = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=Z[2], labels=target))
    ...
    return train, total_loss, loss_CE, X, result, target, accuracy, error
...
if __name__ == '__main__':
    ...
    train, total_loss, loss_CE, X, result, target, accuracy, error = build_neural_network()
    ...
    for i in range(epoch):
        ...
        if (i%display_step == 0):
            ...
            print("training loss: %s"%(train_cost))
            print("valid loss: %s"%(valid_cost))
            print("test loss: %s"%(test_cost))
